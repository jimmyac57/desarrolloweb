...
export const validarCampoVacio = (campo) =>{
    ...
    if(valor.length==0 && !campo.required){
        ...
        return setErrors(`El campo ${campo.name.replace(/\[\]$/, '')} es obligatorio`,campo);
    }else{
        return false;
    }
}
...
export const validarFormato = (regex,regexFeedback, min, max, campo) => {
    ...
    if(valor.length==0){
        ...
        return setErrors(`El ${campo.name.replace(/\[\]$/, '')} debe tener al menos ` + minLength +` caracteres`,campo);
    } 
    else if (!nombreRegex.test(valor)) {
        return setErrors(regexFeedback,campo);
    }
    else if (valor.length > maxLength) {
        return setErrors(`El ${campo.name.replace(/\[\]$/, '')} debe tener como maximo `+ maxLength +` caracteres`,campo);
    } 
    else {
        return setErrors("",campo,false);
    }
